...
function login() {
    ...
    const login = async(data) => {
        try {
            const response = await axios.post("http://localhost:8000/api/v1/branch/loginBranch", {userName: data.userName, password: data.password}, {
                withCredentials: true,
            })
            // console.log(response.data.data);
            if (response) {
                dispatch(authlogin(response.data.data.branch))
        ...
        <div className="bg-gray-100 font-sans mx-44 my-24 border  shadow-2xl">
            <div className="flex ">
                <div className="w-2/5 bg-gray-800 py-44">
                    <h2 className="text-2xl text-white font-semibold mb-4 text-center">ADMIN ?</h2>
                    <p className="text-gray-400 mb-6 text-center w-80 mx-auto">Want to Register new Aptech branch !</p>
                    <div className='text-center'><Link to="/signup"><button className="border px-6 py-2 text-white bg-gray-800 rounded-md hover:bg-gray-900 ">
                        REGISTER BRANCH
                    </button></Link></div>
                </div>
                <div className="w-[745px] bg-white p-10 pb-20 h-[550px]">
                    <img src={logo} alt="Yakazi Logo" className="h-24 mx-auto mb-2" />
                    <h2 className="text-xl  mb-6 text-center ">Welcome to Aptech Login </h2>
                    {error && <div className=' text-center text-red-500 '>*{error}</div>}
                    <form onSubmit={handleSubmit(login)} className='mx-48 mt-4 '>
